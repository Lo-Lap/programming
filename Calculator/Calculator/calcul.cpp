...
int priority(char c)
{
	if ((c == '+') || (c == '-')) return 1;
	if ((c == '*') || (c == '/')) return 2;
	else return 0;//если встретили0 открывающуюся скобку
}
...
bool calcul::check_neg(char c)//обработка отрицательных чисел, если оно первое число в выражении или идёт после открывающей скобочки
{
	if (c == '-')
	{
		if (number.size() == 0)
			return true;
		else if ((operation.top() == '(') && (operation.size() > number.size()))//когда - сразу после скобок, то количесвто операций превышает количество считанных чисел
		{
			return true;
		}
		else return false;
...
calcul::calcul()
{
	...
	while (c=cin.peek())
	{
		...
		if ((c == '+') || (c == '-') || (c == '/') || (c == '*') || (c=='('))
		{
			...
			else//стек с операциями не пуст и приоритет рассматриваемой операции не меньше, чем уже положенная на стек
			{
				operation.push(c);
				cin.ignore();//извлекаем просмотренный символ из потока
			}
